...
private:
  ...
  static_assert(std::is_move_constructible_v<T> || std::is_copy_constructible_v<T>, "Queue element type T must be move or copy constructible");
  static_assert(!std::is_reference_v<T>, "Queue cannot hold reference types.");
  static_assert(!std::is_const_v<T>, "Queue cannot hold const types.");
  ...
  std::unique_ptr<node> wait_for_and_get(std::chrono::milliseconds);
public:
  ...
  bool wait_for_and_pop(T& value, std::chrono::milliseconds);
  ...
  // 9. Add size() function - Not required as it is of no use for MPMC queues
  void clear();
};
} // namespace tsfqueue::__impl
